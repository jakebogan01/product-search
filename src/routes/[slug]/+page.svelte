<script>
	...
	onMount(() => {
		...
		// would normally fetch the specifc product here but I don't know the api endpoint for that
		product = $allProducts.filter((item) => {
			return item.id === data?.slug;
		});
	});
</script>
				...
				<div class="lg:grid lg:auto-rows-min lg:grid-cols-12 lg:gap-x-8">
					<div class="lg:col-span-5 lg:col-start-8">
						<div class="flex justify-between">
							<h1 class="text-xl font-medium text-gray-900">{product[0]?.name}</h1>
							<p class="text-xl font-medium text-gray-900">${product[0]?.price}</p>
						</div>
						<!-- Reviews -->
						<div class="mt-4">
							...
							<div class="flex items-center">
								...
								<div class="ml-4 flex">
									<a href="#" class="text-sm font-medium text-indigo-600 hover:text-indigo-500"
										>See all {product[0]?.ratingCount} reviews</a
									>
								</div>
							</div>
						</div>
					...
					<div class="mt-8 lg:col-span-7 lg:col-start-1 lg:row-span-3 lg:row-start-1 lg:mt-0">
						...
						<div class="grid grid-cols-1 gap-8">
							<img
								src={product[0]?.thumbnailImageUrl || product[0]?.imageUrl}
								alt={product[0]?.name}
								class="lg:col-span-2 lg:row-span-2 rounded-lg w-full object-contain object-top"
							/>
						</div>
					</div>
								...
								<fieldset class="mt-2">
									...
									<div class="flex items-center space-x-3">
										...
										>
											...
											<span
												aria-hidden="true"
												class="h-8 w-8 bg-red-500 rounded-full border border-black border-opacity-10"
											/>
										</label>
									</div>
								</fieldset>
